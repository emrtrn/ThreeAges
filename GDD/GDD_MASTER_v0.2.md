...
- Üç çağ: Yerleşim, Kasaba, Krallık — **ikisi bağlı**; Krallık ThirdAge sanat
  kaynağına bağlı olarak ertelendi (`SL-007`)
- **Çağ ve bina seviyesi bağımsız iki eksendir** (`02 §25`): çağ sanat ailesini
  ve yeni yapı/birlik açılımını belirler, seviye (Lv1–3) çağın içinde işler
- Seviye yükseltmesi **bina instance'ı başınadır** ve çağ kapısı yoktur
- Çağ atlayınca tüm yapılar yeni çağın Lv1 modeline geçer ve seviyeleri sıfırlanır
...
- Ana yapılar her çağın içinde üç seviye kullanır (çağ × seviye model matrisi)
...
- Fog of war Ürün C için koşullu sistemdir; minimap küçük sahne ölçeği nedeniyle kapsam dışıdır (`SCOPE_LOG.md` SL-006)
...
- [x] Minimap küçük sahne ölçeği nedeniyle kapsamdan çıkarıldı; fog bağımsız değerlendirilecek. (`SCOPE_LOG.md` SL-006)
...
- Fog, Refah, save/load ve bölgesel zafer Ürün A dışında tutulur; minimap tüm ürün kapsamından çıkarılmıştır (`SCOPE_LOG.md` SL-006).
...
Vertical slice tamamlanmış sayılırsa (yetkili kaynak: `13_VERTICAL_SLICE_PRODUCTION_PLAN_v0.2.md` §107; **save/load ve minimap kapsam dışıdır**, fog ve bölgesel zafer koşulludur):
...
- [ ] Fog of war — koşullu sistemdir; yalnızca kabul edildiyse çalışır (bkz. 13 v0.2 §59, §107).
- [x] Minimap — küçük sahne ölçeği nedeniyle kapsam dışıdır; navigasyon alternatifleri UI planında tanımlıdır (`SCOPE_LOG.md` SL-006).
