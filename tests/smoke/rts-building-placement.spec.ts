...
test("RTS Phase 7 a box-selected group takes a move order and every unit finishes it", async ({ page }) => {
  ...
  // Picking a formation is selection state only: the panel exposes all six
  // diagrams for a combat group, starts on Hat, and changing it must not issue a
  // movement command by itself. The existing move assertion below then proves the
  // ordinary move path remains intact after the UI choice.
  const formation = panel.locator(".rts-selection-formation");
  await expect(formation).toBeVisible();
  await expect(formation.locator(".rts-selection-formation-option")).toHaveCount(6);
  await expect(formation.locator('[data-rts-formation="line"]')).toHaveAttribute("data-rts-active", "true");
  await formation.locator('[data-rts-formation="column"]').click();
  await expect(formation.locator('[data-rts-formation="column"]')).toHaveAttribute("data-rts-active", "true");
