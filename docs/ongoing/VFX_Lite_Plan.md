...
İlk tetikleyici (ateş VFX'i) geldiğinde **Faz 6a — tek sprite doku** yapıldı;
kalan alt maddeler hâlâ ihtiyaç bekliyor.
...
- [~] Sprite texture / alpha texture + particle material asset ailesi ve
      ...
      **Faz 6a (tek sprite doku) TAMAMLANDI (4 Temmuz 2026):** aşağıya bkz.
      `materialId` + `M_FX_*` particle material asset ailesi hâlâ açık.
- [x] Texture atlas ve flipbook. **Faz 6b TAMAMLANDI (4 Temmuz 2026):** aşağıya bkz.
...
### Faz 6a — Tek Sprite Doku  ✅ TAMAMLANDI (4 Temmuz 2026)

Point sprite renderer'ına isteğe bağlı doku örneklemesi eklendi; ateş VFX'i bu
yolla yapıldı. Kapsam bilinçli olarak dar tutuldu (tek kare sprite): SubUV
flipbook (Faz 6b), quad/billboard, ve `materialId` + `M_FX_*` particle material
asset ailesi hâlâ açık.

- [x] `ParticleRendererBlock.texture: string | null` (manifest **texture asset
      id**, materyal kalıbıyla aynı) — `engine/vfx/particleEffectTypes.ts`.
      `RuntimeParticleEffect.texture?: string` collapse'ta taşınır.
      `normalizeRenderer` `readTextureRef` ile okur (boş/whitespace → null);
      validator normalizer'ı tek kaynak kullandığından `validateEffectAsset`
      otomatik kapsar.
- [x] Renderer shader'ı `uMap`+`uHasTexture` uniform'ları + `gl_PointCoord`
      (Y-flip) örneklemesi; doku yoksa mevcut prosedürel yumuşak daireye düşer.
      `ParticleEffect` ctor'una opsiyonel 3. arg `textureUrl` (async `TextureLoader`,
      `dispose`'da texture da bırakılır) — `engine/render-three/particleEffect.ts`.
- [x] Runtime boundary: `VfxSubsystem`'e `resolveTextureUrl` callback'i
      (`resolveEffectUrl` kalıbı); `acquire` id→URL çözer, `ParticleEffect`'e verir
      (havuz reuse'da texture sabit). `RuntimeSceneApp` `textureUrlById` map'ini
      manifest'ten kurar ve enjekte eder.
- [x] Editör: Renderer grubuna sprite-texture picker (`<none>` + texture asset'leri);
      `assets` opsiyonu `EditorUi`'den geçirilir; preview viewport `resolveTextureUrl`
      ile dokuyu çözer, böylece editörde ve `/` route'unda aynı görünür.
- [x] `fire` preset'i (`fire-01-2` Kenney soft blob, additive, turuncu→koyu kırmızı,
      loop) + starter asset `FX_Fire_Loop.effect.json` + manifest kaydı
      (`starter-fx-fire-loop`, `assetType: "effect"`).
- [x] Headless test: `renderer.texture` normalize/collapse/validate round-trip +
      boş→null + schema-1 texture-siz davranışı.

**Kabul kriteri:** Doku referanslı bir effect asset'i hem editör preview'inde hem
`/` route'unda dokulu sprite ile oynar; doku-suz asset'ler değişmez. ✅ `build:verify`
yeşil (tsc + vite build + 586 engine check + verify:dist --strict), `check:assets`
errors=0. **Kalan:** canlı browser smoke (kullanıcı) — ateş asset'ini editörde aç,
sprite dokusunu gör; `/` route'unda emitter'ı oynat.

### Faz 6a karar kaydı (4 Temmuz 2026)

**Doku, texture asset **id**'si olarak saklanır (path değil).** Materyal ailesi
(`materialAssets.ts`) texture'ları asset id ile referanslar; aynı sözleşme izlendi.
id→URL çözümü manifest'i bilen app sınırında (`RuntimeSceneApp` / editör) yapılır;
`engine/vfx` ve `RuntimeParticleEffect` manifest-free kalır, `ParticleEffect`
yalnızca çözülmüş URL'i alır ve `TextureLoader` ile yükler.

**Renderer flat kontratı korundu.** Faz 1'deki "staged bridge" bozulmadı: texture
tek bir opsiyonel alan olarak flat `RuntimeParticleEffect`'e eklendi; zengin
schema-2 → collapse → flat hattı aynı. SubUV/flipbook (kare animasyonu),
quad/billboard yönelimi ve particle material asset ailesi (`materialId`) bilinçle
6a dışında bırakıldı — nokta sprite tek kareyi örnekler.

**Renk yönetimi "raw" bırakıldı.** Mevcut particle shader'ı (Faz 2/3) çıktı renk
uzayı dönüşümü yapmayan ham bir ShaderMaterial; texture çarpımı da aynı ham hatta
eklendi (doku `colorSpace` set edilmez). Additive blend siyah zemini yutar, bu
yüzden gri-siyah Kenney sprite'ları turuncu tint ile ateş gibi görünür. Gerçek
color-management gerekirse ayrı bir adımda ele alınır.

### Faz 6b — SubUV Flipbook  ✅ TAMAMLANDI (4 Temmuz 2026)

Point sprite renderer'ı sprite-sheet (flipbook) atlaslarını destekliyor: doku
`cols × rows` kareye bölünür ve her parçacık yaşamı boyunca kareleri sırayla
oynatır. `T_Fire_SubUV` (6×6) ve `T_Explosion_SubUV` (6×6) artık gerçek
animasyonlu ateş/patlama olarak çalışır.

- [x] `SubUVGrid { cols, rows }` tipi + `ParticleRendererBlock.subUV: SubUVGrid`
      (her zaman var, varsayılan `{1,1}`) + `RuntimeParticleEffect.subUV?`
      (yalnız `cols*rows > 1` iken emit). `{1,1}` = tüm doku (tek sprite) — 6a
      artık flipbook'un dejenere hâli. `normalizeRenderer` `readSubUV` ile
      pozitif tamsayıya yuvarlar + `[1,32]`'ye clamp'ler; validator normalizer'ı
      kullandığı için otomatik kapsar.
- [x] Renderer: per-particle `aLifeT` (yaşam kesri) attribute'ü → `vLifeT`
      varying; fragment `frame = floor(vLifeT * cols*rows)` ile atlas alt-kare
      UV'sini hesaplar. `uSubUV` uniform'u `definition.subUV`'den. **`flipY=false`**
      (tüm particle dokuları) → `gl_PointCoord` (sol-üst) doğrudan görüntü uzayına
      düşer, atlas kare 0 = sol-üst hücre; tek-sprite Y-flip'i kaldırıldı.
- [x] Presetler: `fireAnimated` (t-fire-subuv 6×6 loop) + `explosion`
      (t-explosion-subuv 6×6 one-shot burst). Starter asset'ler
      `FX_Fire_SubUV.effect.json` + `FX_Explosion.effect.json` + manifest kayıtları
      (`starter-fx-fire-subuv`, `starter-fx-explosion`). Mevcut 6 preset'in +
      schema-1 inline renderer'ı `subUV: {1,1}` kazandı.
- [x] Editör: Renderer grubunda doku seçiliyken **Flipbook Cols/Rows** sayı
      alanları (`renderer.subUV.cols`/`rows`, 1–32); texture değişince re-render.
- [x] Headless test: subUV normalize/clamp (6.4→6, 0→1, 999→32), collapse
      (frames>1 emit / 1×1 omit), `validateEffectAsset` round-trip. Gerçek asset
      doğrulaması: iki SubUV starter parser'dan geçti (texture id + 6×6 + PNG var).

**Kabul kriteri:** SubUV atlaslı bir effect asset'i kareleri animasyonlu oynatır;
1×1 (ve doku-suz) efektler değişmez. ✅ `build:verify` yeşil (tsc + vite build +
587 engine check + verify:dist --strict), `check:assets` errors=0. **Kalan:**
canlı browser smoke (kullanıcı) — `FX_Fire_SubUV` / `FX_Explosion` editörde/`/`
route'unda kareli animasyon oynatmalı.

### Faz 6b karar kaydı (4 Temmuz 2026)

**Kare animasyonu yaşam-tabanlı (age fraction).** Kare, parçacığın yaşam kesrinden
(`age/lifetime`) türetilir — flipbook her parçacığın ömründe bir kez oynar. Bu hem
loop ateşe (her parçacık yükselirken alevi döner) hem one-shot patlamaya (patlama
bir kez oynar) uyar. Ayrı bir sabit-FPS/loop kare modu YAGNI gereği eklenmedi;
gerekirse sonra gelir. `aLifeT` ayrı attribute olarak eklendi (alpha formülüne
bağlanmadı) → ileride fadeIn/out gelince kırılmaz.

**`subUV` her zaman `{cols,rows}` (nullable değil).** `texture: string|null`'ın
aksine subUV `{1,1}` varsayılanıyla hep mevcut; bu tek-sprite'ı flipbook'un
dejenere hâli yaparak shader'da tek kod yolu sağlar (`uHasSubUV` bayrağı gerekmez).
Flat runtime kontratı yalın kalsın diye collapse yalnız animasyonluyken (`>1`) emit
eder.
