...
- 2026-08-06 — Faz 0 + Faz 1 dar çekirdeği başlatıldı.
  ...
  - [ ] Faz 3 `wood-dark` üretim seti, kullanıcı geri bildirimiyle C varyantına yenilendi: parke/tahta derzleri yok, kesintisiz doğal ahşap yüzeyi ve daha açık orta-kestane tonu var. Deterministik 1024² BC/N/ORM seti, manifest texture kayıtları ve `M_TA_Wood_Dark` korunuyor; yeni görünümün görsel kabulü henüz açık.
  - [x] `wood-dark` Material Editor browser smoke, kayıtlı BC/normal/ORM setini açıp `Ready.` durumuna ulaştı; sayfa hatası üretmedi.
  - [ ] Faz 3 `wood-light` üretim seti, kullanıcı geri bildirimiyle C varyantına yenilendi: parke/tahta derzleri yok, kesintisiz doğal ahşap yüzeyi var. Deterministik 1024² BC/N/ORM seti, manifest texture kayıtları ve `M_TA_Wood_Light` korunuyor; yeni görünümün görsel kabulü henüz açık.
  - [x] Faz 3 iki metal üretim seti hazır: koyu dövme demir `M_TA_Metal_Dark` ve eskitilmiş bronz `M_TA_Metal_Bronze`, ayrı BC/N/ORM dosyaları ve manifest kayıtlarıyla `Textures`/`Materials` altında yer alır. Model slotlarına henüz otomatik atama yapılmadı.
